...
JINZAI_CONFIG = REPO_DIR / "jinzai-kanri" / "config.py"
...
# ===================================================================
# マスター設定ページ（部署・役職・職種区分の編集）
# ===================================================================
def _parse_config_list(content: str, var_name: str) -> list[str]:
    """config.pyの内容から指定リスト変数をパースする。"""
    for line in content.splitlines():
        stripped = line.strip()
        if stripped.startswith(f"{var_name}") and "=" in stripped:
            rhs = stripped.split("=", 1)[1].strip()
            try:
                return json.loads(rhs.replace("'", '"'))
            except Exception:
                pass
    return []


def _update_config_list(content: str, var_name: str, new_list: list[str]) -> str:
    """config.pyの内容のうち、指定リスト変数の行を新しい値で置き換える。"""
    list_str = json.dumps(new_list, ensure_ascii=False)
    lines = content.splitlines()
    for i, line in enumerate(lines):
        if line.strip().startswith(f"{var_name}") and "=" in line:
            lines[i] = f'{var_name} = {list_str}'
            break
    return "\n".join(lines) + "\n"
...
def page_master_settings() -> None:
    ...
    content = JINZAI_CONFIG.read_text(encoding="utf-8")
    departments = _parse_config_list(content, "DEPARTMENTS")
    positions = _parse_config_list(content, "POSITIONS")
    roles = _parse_config_list(content, "ROLES")

    if st.session_state.get("master_flash"):
        st.success(st.session_state.pop("master_flash"))

    # --- 職種区分 ---
    st.subheader("職種区分（ROLES）")
    roles_text = st.text_area(
        "1行に1つずつ入力", value="\n".join(roles), height=120, key="edit_roles",
        help="人材管理アプリの社員登録・編集で選択肢として表示されます。")

    # --- 部署 ---
    st.subheader("部署（DEPARTMENTS）")
    depts_text = st.text_area(
        "1行に1つずつ入力", value="\n".join(departments), height=120, key="edit_depts")

    # --- 役職 ---
    st.subheader("役職（POSITIONS）")
    ...
    if st.button("💾 保存してGitHubに反映", type="primary", use_container_width=True, key="save_master"):
        ...
        updated = content
        updated = _update_config_list(updated, "ROLES", new_roles)
...
st.sidebar.divider()
PAGE_MODES = ["プロジェクト管理", "マスター設定"]
page_mode = st.sidebar.radio("ページ", PAGE_MODES, key="dev_page_mode")

if page_mode == "マスター設定":
    page_master_settings()
else:
    project = _sidebar()
    if project:
        main_view(project)
