...
9. **設計判断をしたら docs/saas/adr/ に1ファイル追加**する。
   ファイル名は `ADR-0001-lowercase-slug.md`（4桁の番号 + 半角英小文字・数字・ハイフン）。
   **番号は重複させない。** 並行して進んだ作業が同じ番号を取る事故が5回起きているため、
   `tests/test_adr_numbering.py` が CI で落とす。落ちたらメッセージに出る番号に振り直す。
   ぶつかりやすいので**番号はマージ直前に確定させる**のが安全。
   既に重複している 0022 / 0070 / 0080 / 0082 は凍結し、振り直さない（理由は同テストに記載）

## リファクタリング作業の進め方

`docs/learning/refactoring/` に1回ずつ記録しながら進める。**読み手はリファクタリング
初心者**という前提で、次を守る（詳細は同フォルダの README.md）。

- **図を必ず付ける**（1回1枚以上・1図1概念・`figures/` にインタラクティブ HTML）
- **専門用語は初出で一言の定義を置く**。英語の用語は日本語を併記する
- **なぜそうするかを先、手順を後**。コードは「この行が何をしているか」から説明する
- **数字で示す**（「多い」ではなく「401 箇所」）
- **振る舞いを変える変更と、変えない変更を同じコミットに混ぜない**
- **直す前と後の出力を機械で突き合わせる**。「たぶん同じ」で進めない
...
│   ├── schedules/   … Phase, Milestone, Assignment                    [M7]
│   └── bids/        … BidProject, BidCost, Qualification, ...        [M8]
├── tests/           … テスト (越境テスト必須, 50件)
├── Dockerfile
├── docker-compose.yml
└── manage.py
```
