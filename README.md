...
1. 案件の詳細を開き、直したい欄の「**直す**」を押す（案件情報は「日付を直す」、参加資格判定は「読み取りを直す」、重要日程は「日程を直す」）
2. 直して「**直す**」を押す

- **重要日程**は行ごとに、項目名・日時・補足を直せます。項目名を空にするとその行が消え、空の行に入れると足せます
- **入札期限**は時刻まで入れられます（17時まで、など）
- 直した項目には「**手直しあり**」と出て、**公告を取り込み直しても書き換わりません**
