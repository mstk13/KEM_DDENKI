# 設計ドキュメント

## システム再設計（2026-09〜）

- frontend_redesign_spec_v2.md … システム再設計仕様書 v2.0（最新・実装はこれに従う）
- frontend_redesign_spec.md … v1.1（レスポンシブ方針・htmx採用・評価統合などの決定記録。v2.0に継承済み）
- redesign_proposal.html … 社内共有用の図解資料（ブラウザで開く）
- redesign_v2_requirements.md … v2.0 を実装に落とすための要求定義（ユーザーストーリー・受け入れ基準・未決事項・PR 分割）
- redesign_v2_decisions.md … 判断が必要な事項ごとの事実・選択肢・推奨・止まっている作業（結論もここに記入する）

実装時の参照順: v2.0 → 不明点があれば v1.1 の決定記録 → 共有資料の図。
v2.0 と既存 ADR の食い違いは redesign_v2_requirements.md の「未決事項」で扱い、決まったものから ADR（docs/saas/adr/）に残す。

## クラウドDB移行（2026-09〜）

- cloud_db_migration_plan.md … 案C（DB は社内で暗号化、暗号化したバックアップとアップロードファイルをクラウド）の対策と分担・棚卸し・要件・料金調査・試験移行・未決事項
  （方針は ADR-0046。当初の「DB をまるごとクラウド」は ADR-0044、社内AIの扱いは ADR-0042）

## 初期設計（2026-08 作成）

- 要件定義書.md / DB設計書.md / 画面設計書.md / 技術選定書.md / AI設計.md
- 設計判断の根拠書.md / TM向けQ&A集.md / 技術用語集.md

初期設計は8モジュール構成の前提で書かれている。画面構成・権限・日報の扱いは再設計 v2.0 が優先する。
